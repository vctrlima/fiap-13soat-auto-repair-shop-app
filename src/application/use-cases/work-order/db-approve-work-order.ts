...
export class DbApproveWorkOrder implements ApproveWorkOrder {
  ...
  async approve(
    params: ApproveWorkOrder.Params,
  ): Promise<ApproveWorkOrder.Result> {
    const workOrder = await this.updateWorkOrderRepository.update({
      id: params.id,
      status: Status.Approved,
    });
    workOrderCompletedCounter.add(1, { "work_order.status": "approved" });
    if (env.mailing.enabled) {
      await this.sendEmailTransporter.send({
        ...
        html: notificationTemplate.update(
          workOrder,
          `${env.host}/api/work-orders/${workOrder.id}`,
        ),
      });
    }
    return workOrder;
