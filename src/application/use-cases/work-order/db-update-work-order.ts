  ...
  async update(params: UpdateWorkOrder.Params): Promise<UpdateWorkOrder.Result> {
    ...
    if (result.status === Status.WaitingApproval) {
      await this.sendEmailTransporter.send({
        ...
        html: notificationTemplate.approvalRequest(result, `${env.host}/api/work-orders/${result.id}`),
      });
    } else {
      await this.sendEmailTransporter.send({
        to: result.customer.email,
        ...
        html: notificationTemplate.update(result, `${env.host}/api/work-orders/${result.id}`),
      });
    }
    return result;
  }
