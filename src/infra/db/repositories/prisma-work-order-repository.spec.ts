...
describe("PrismaWorkOrderRepository", () => {
  ...
  it("should create a new work order with services and parts", async () => {
    ...
    jest
      ...
      .mockResolvedValueOnce([mockData.serviceData]);
    jest
      .spyOn(prisma.partOrSupply, "findMany")
      .mockResolvedValueOnce([mockData.partData]);
    jest
      .spyOn(prisma.workOrder, "create")
      .mockResolvedValueOnce(createdWorkOrder as any);
  ...
  it("should calculate budget correctly with multiple services and parts", async () => {
    ...
    const expectedBudget =
      mockData.serviceData.price +
      additionalService.price +
      mockData.partData.price +
      additionalPart.price;
    const createdWorkOrder = {
      id: faker.string.uuid(),
      customerId: params.customerId,
      vehicleId: params.vehicleId,
    ...
    jest
      .spyOn(prisma.service, "findMany")
      .mockResolvedValueOnce([mockData.serviceData, additionalService]);
    jest
      .spyOn(prisma.partOrSupply, "findMany")
      .mockResolvedValueOnce([mockData.partData, additionalPart]);
    jest
      .spyOn(prisma.workOrder, "create")
      .mockResolvedValueOnce(createdWorkOrder as any);
  ...
  it("should return a work order by id", async () => {
    ...
    const result = await prismaWorkOrderRepository.getById({
      id: workOrder.id,
    });
  ...
  it("should throw NotFoundError if work order does not exist", async () => {
    ...
    await expect(
      prismaWorkOrderRepository.getById({ id: faker.string.uuid() }),
  ...
  it("should update work order status and recalculate budget", async () => {
    ...
    jest
      .spyOn(prisma.workOrder, "findUnique")
      .mockResolvedValueOnce(existingWorkOrder as any);
  ...
  it("should delete an existing work order", async () => {
    ...
    jest
      .spyOn(prisma.workOrder, "findUnique")
      .mockResolvedValueOnce(workOrder as any);
    (prisma as any).workOrderPartOrSupply = {
      deleteMany: jest.fn().mockResolvedValueOnce({}),
    };
    (prisma as any).workOrderService = {
      deleteMany: jest.fn().mockResolvedValueOnce({}),
    };
    (prisma as any).serviceMetrics = {
      deleteMany: jest.fn().mockResolvedValueOnce({}),
    };

    jest
      .spyOn(prisma.workOrder, "delete")
      .mockResolvedValueOnce(workOrder as any);

    await prismaWorkOrderRepository.delete({ id: workOrder.id });

    expect(
      (prisma as any).workOrderPartOrSupply.deleteMany,
  ...
  it("should throw NotFoundError if work order to delete does not exist", async () => {
    ...
    await expect(
      prismaWorkOrderRepository.delete({ id: faker.string.uuid() }),
    ).rejects.toThrow(NotFoundError);
  });
});
